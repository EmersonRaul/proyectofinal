...
//
    ...
    public void componentesClientes() {
        ...
        ActionListener nuevo_ini = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                txtValor7.setText(Genero_Cliente.toUpperCase());
        ...
        if (Nit.equals("") || Nombre.equals("") || Edad.equals("") || Genero.equals("")) {
            ...
            if (duplicadosDeNit(Nit)) {
                ...
                if (vacio) {
                    ...
                    JOptionPane.showMessageDialog(null, "!!Ya No Se Pueden Guardar Mas Clientes!!!");
                }
            }
        }
    ...
    public void componentesGraficas() {
        // ------------- Panel De Clientes
        PanelGraficador = new JPanel();
        PanelGraficador.setLayout(null);
        this.getContentPane().add(PanelGraficador);
        
        JLabel lblClientes = new JLabel("Stored clients");//clientes almacenados
        lblClientes.setBounds(10, 10, 150, 15);
        PanelGraficador.add(lblClientes);
        ...
        JScrollPane Clientes_Barra = new JScrollPane(clientes_tabla);
        Clientes_Barra.setBounds(25, 35, 350, 250);
        PanelGraficador.add(Clientes_Barra);
        
        JButton btsdoficar = new JButton("Modificar");
        btsdoficar.setBounds(270, 350, 150, 30);
        btsdoficar.setBackground(Color.BLUE);
        btsdoficar.setForeground(Color.WHITE);
        PanelGraficador.add(btsdoficar);
        ActionListener modi = new ActionListener() {
            
            @Override
            
            public void actionPerformed(ActionEvent e) {
                PanelGraficador.setVisible(false);
                PanelClientes.setVisible(true);
                String Buscar = JOptionPane.showInputDialog("Ingresa El Nit A Buscar");
                
                int posicion = -1;
                for (int i = 0; i < 100 - 1; i++) {
                    if (misClientes[i] != null) {
                        
                        if (misClientes[i].getNit().equals(Buscar)) {
                            
                            Buscar = JOptionPane.showInputDialog("Cambiar Nit");
                            misClientes[i].setNit(Buscar);
                            String Nombre = JOptionPane.showInputDialog("Cambiar Nombre");
                            misClientes[i].setNombre(Nombre);
                            String Edad = JOptionPane.showInputDialog("Cambiar Nombre");
                            misClientes[i].setEdad(Edad);
                            String Genero = JOptionPane.showInputDialog("Cambiar Nombre");
                            misClientes[i].setGenero(Genero);
                            posicion = i;
                            break;
                        }
        ...
        btsdoficar.addActionListener(modi);

        //Vista Informativa
        JButton btsinfor = new JButton("Informacion");
        btsinfor.setBounds(100, 400, 150, 30);
        btsinfor.setBackground(Color.BLUE);
        btsinfor.setForeground(Color.WHITE);
        PanelGraficador.add(btsinfor);
        ActionListener mas = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                PanelGraficador.setVisible(false);
                PanelClientes.setVisible(true);
                String Mega = JOptionPane.showInputDialog("Ingresa El Nit del Cliente");
                
                int posicion = -1;
                for (int i = 0; i < 100 - 1; i++) {
                    if (misClientes[i] != null) {
                        
                        if (misClientes[i].getNit().equals(Mega)) {
                            posicion = i;
                            break;
                        }
                    }
                ...
                if (posicion != -1) {
                    JOptionPane.showMessageDialog(null, "Nombre: " + misClientes[posicion].getNombre() + "\n" + "  Edad: " + misClientes[posicion].getEdad() + "\n" + "  Genero: " + misClientes[posicion].getGenero());
                    //return "Nombre: " + misClientes[posicion].getNombre() + "  Edad: " + misClientes[posicion].getEdad() +  "  Genero: " + misClientes[posicion].getGenero();
        ...
        btsinfor.addActionListener(mas);

        //Bonton Volver
        JButton btsvol = new JButton("Volver");
        btsvol.setBounds(100, 350, 150, 30);
        btsvol.setBackground(Color.BLUE);
        btsvol.setForeground(Color.WHITE);
        PanelGraficador.add(btsvol);
        ActionListener volver = new ActionListener() {
        ...
        btsvol.addActionListener(volver);
        
    
        //  Tabla De pastel
        DefaultPieDataset Grafico_Pastel_Edades = new DefaultPieDataset();
        Grafico_Pastel_Edades.setValue("Masculino", 6);
        Grafico_Pastel_Edades.setValue("Femenino", 4);
        
        JFreeChart Circulas_Grafico = ChartFactory.createPieChart("Genero", Grafico_Pastel_Edades,true,true,false);
        ChartPanel Panel_Circular = new ChartPanel(Circulas_Grafico);
        
        Panel_Circular.setBounds(400, 20, 300, 250);
        PanelGraficador.add(Panel_Circular);
